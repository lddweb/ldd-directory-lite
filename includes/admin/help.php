...
function ldl_contextual_help() {
    ...
    $content = '<p>' . __('The Appearance tab allows you to customize some basic presentation features to help match the look and feel of the directory with your WordPress theme.', 'lddlite') . '</p>' .
        '<p>' . __('We tried to design the plugin in such a way that it would fit in with most themes straight out of the box, but given the sheer number of available themes or the fact that you may be using this plugin within a completely bespoke design, it is impossible to anticipate every situation.', 'lddlite') . '</p>' .
        '<p><strong>' . __('Colors', 'lddlite') . '</strong>' . '<br>' . __('Unless otherwise noted, these will always be in the order of normal state, hover state, and foreground. In some cases these will affect multiple elements with and without hover states, so always be sure to check after making changes.', 'lddlite') . '</p>' .
        '<p><strong>' . __('Template Files', 'lddlite') . '</strong>' . '<br>' . __('Most of the presentation is generated through the use of template files, just like those used in a WordPress theme.', 'lddlite') . '</p>' .
        ...
        '<p>' . __("Any presentation not controlled by a template should have a WordPress filter attached to it. These will always be in the format of 'lddlite_filter_presentation_{name}' and developer documentation will be available on our web site soon.", 'lddlite') . '</p>';
    $screen->add_help_tab(array(
